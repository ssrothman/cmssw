...
public:
  ...
  void beginRun(const edm::Run&, const edm::EventSetup& iSetup) override {
    ...
    hgcalTools_.setGeometry(*(caloGeom_.product()));
  }
  ...
  GlobalPoint positionFromDetId(const DetId& id, int& subdet) const{
    switch(id.det()){
      ...
      case DetId::Ecal:
        subdet = id.subdetId();
        return getEcalHcalHitPosition(id);
      case DetId::HGCalEE:
      case DetId::HGCalHSi:
      case DetId::HGCalHSc:
        subdet = id.subdetId();
        return hgcalTools_.getPosition(id);
...
protected:
  ...
  hgcal::RecHitTools hgcalTools_;

  GlobalPoint getEcalHcalHitPosition(const DetId& id) const{
...
typedef HitPositionTableProducerT<PCaloHit, false> SimHitPositionTableProducer;
...
DEFINE_FWK_MODULE(SimHitPositionTableProducer);
